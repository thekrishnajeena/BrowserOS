...
declare namespace chrome.browserOS {
  ...
  type Key = 'Enter' | 'Delete' | 'Backspace' | 'Tab' | 'Escape' | 
  ...
  function sendKeys(
    tabId: number,
    key: 'Enter' | 'Delete' | 'Backspace' | 'Tab' | 'Escape' | 'ArrowUp' | 'ArrowDown' | 'ArrowLeft' | 'ArrowRight' | 'Home' | 'End' | 'PageUp' | 'PageDown',
    callback: () => void
  ): void;
  ...
  type SectionType = 'main' | 'navigation' | 'footer' | 'header' | 'article' | 
    'aside' | 'complementary' | 'contentinfo' | 'form' | 'search' | 'region' | 'other';
